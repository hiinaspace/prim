...
internal static class GameSession
{
    ...
    public static async Task<int> Run(bool autoVr, string version, IEnumerable<string>? extra = null)
    {
        ...
        try {
            ...
            if (!autoVr) { start.ArgumentList.Add("--"); start.ArgumentList.Add("--desktop"); }
            ...
            Paths.AtomicJson(Path.Combine(dir, "session.json"), new { Version = version, Mode = autoVr ? "Auto" : "Desktop", Start = began, Pid = child.Id });
            Program.Log.Write($"Started prim {version}, {(autoVr ? "Auto" : "desktop")}, session {session}");
            ...
            Paths.AtomicJson(Path.Combine(dir, "session.json"), new { Version = version, Mode = autoVr ? "Auto" : "Desktop", Start = began, End = DateTimeOffset.UtcNow, Exit = child.ExitCode });
